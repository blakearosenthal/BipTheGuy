...
//  BipTheGuy
//
//  Created by Rosenthal, Blake Alexander on 9/23/21.
...
class ViewController: UIViewController {
    ...
    @IBAction func photoOrCameraPressed(_ sender: UIButton) {
        ...
        let photoLibraryAction = UIAlertAction(title: "Photo Library", style: .default) { (action) in self.accessPhotoLibrary()
        }
        
        let cameraAction = UIAlertAction(title: "Camera", style: .default) { (action) in self.accessCamera()
        }
...
extension ViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func accessCamera() {
        if UIImagePickerController.isSourceTypeAvailable(.camera) {
            ...
            showAlert(title: "Camera Not Avaible", message: "There is no camera avaible on this device.")
        }
    }
}
